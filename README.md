...
针对 Steam 端游戏 BloodStrike（译名：血战突袭）登录时提示「由于地区限制暂时无法登录」的解决补丁工具，一个非常简易的 IP 地区检测绕过工具。
...
- **双 DLL 支持** — 同时查找Steam端的 `NtUniSdkSteam.dll` 和官网Win端的 `NtUniSdkMpayOversea.dll`，自动定位存在的那个
